...
library(calibrate)
#label down genes
with(subset(tabela, logFC < -2 & minuslog10P >2), textxy(logFC, minuslog10P, labs=Gene.symbol, cex=.8, col="green")) 
#label up genes
with(subset(tabela, logFC > 2 & minuslog10P >2), textxy(logFC, minuslog10P, labs=Gene.symbol, cex=.8, col="red")) 
...
exp1<- expr
samplesinfo <- GSE21942_f
samplesinfo$Class <- as.character(samplesinfo$Class)
...
exp1 <- exp1[!is.na(exp1$Symbol), ]
exp1 <- exp1[exp1$Symbol != "", ]
